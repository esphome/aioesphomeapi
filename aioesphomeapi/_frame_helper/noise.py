...
class APINoiseFrameHelper(APIFrameHelper):
    ...
    def _handle_hello(self, msg: memoryview | bytes) -> None:
        """Perform the handshake with the server."""
        server_hello = bytes(msg)
        if not server_hello:
            self._handle_error_and_close(
    ...
    def _handle_handshake(self, msg: memoryview | bytes) -> None:
        if msg[0] != 0:
    ...
    def _handle_frame(self, frame: memoryview | bytes) -> None:
    ...
    def _handle_closed(self, frame: memoryview | bytes) -> None:  # pylint: disable=unused-argument
